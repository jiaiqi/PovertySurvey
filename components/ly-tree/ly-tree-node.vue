<template>
	<view
		...
		v-show="node.visible"
		:class="{ 'is-expanded': expanded, 'is-hidden': !node.visible, 'is-checked': !node.disabled && node.checked }"
		role="treeitem"
		name="LyTreeNode"
		ref="node"
	>
		<!-- <uni-swipe-action style="background-color: #fff;"> -->
			<!-- <uni-swipe-action-item :options="options" @click="editCate(node, $event)" class="swipe-item"> -->
				<view class="ly-tree-node__content" :class="{ 'is-current': node.isCurrent && highlightCurrent }" :style="{ 'padding-left': (node.level - 1) * indent + 'px' }">
					<text
						...
						:class="[
							{ 'is-leaf': node.data.is_leaf === '是' ? true : false, expanded: node.data.is_leaf === '是' ? false : true && node.expanded },
...
export default {
	...
	methods: {
		editCate(item, $event) {
			if ($event.index == 0) {
				...
				this.tree.$emit('item-detai', data);
			}
	...
	mounted() {
		if (this.node.data.no === '610800000000') {
