<template>
	<view class="width100 overflow_x_hidden" v-show="onReady">
		<view class="width100 transition_border_point6s border_radius_4px cu-form-group" :class="[item.animationType||animationType||'']"
		 :style="
		...
		'border: 1px solid ' + (verifyStatusObj[(item.variableName)||(onLoadData + index)]||'rgba(0,0,0,0)') + ';'"
		 v-for="(item, index) in inputsArray" :key="item.title" :id="'Id_' + (item.variableName||index)" v-show="!item.hide">
			<!-- segmentationTitle -->
			<view class="width100 flex_column view_Title_Left " :style="classObj.segmentationTitle" v-if="item.segmentationTitle">
				{{item.segmentationTitle}}
			</view>
			<!-- elements -->
			<view class="flex_column width100 " :style="{
			...
			'border-top': item.border_top||'none'}">
				<!-- title -->
				<view class="marginV5 flex_row_s_c title" :class="[(titleFixedWidth || item.type==='text')?'width20':'Flex1_5', (titleFixedWidth || item.type==='text')?'':'maxWidth40', classObj.titleLayout]"
				...
				<view :class="[(item.type==='editor'?'width100': ((titleFixedWidth || item.type==='text')?classObj.contentsWidth:'Flex4')), classObj.contentsLayout]">
							...
							<label class="fontColor666666 flex_row_none_c" :style="classObj.contentFontSize + classObj.padding1 + classObj.marginRight2"
							 v-for="(checkboxItem, checkboxIndex) in item.itemArray" :key="checkboxIndex">
								...
								inputsObj[item.variableName||(onLoadData+index)].status[checkboxIndex]===true"
								 :disabled="checkboxItem.disabled" :color="checkboxItem.color||item.color" :style="'transform: scale(' + (item.scale||'.8') + ');'" class="checkbox"/>
								<view class="flex_row_none_c checkbox_label">{{checkboxItem.name}}</view>
							</label>
						</checkbox-group>
					</view>
					<!-- textarea -->
					<view :class="[classObj.contentLayout]" :style="classObj.contentWidth" v-else-if="item.type&&item.type=='textarea'">
						...
						'border': '1px solid ' + (focusStyleObj[(item.variableName||(onLoadData+index))]?(item.focusBorderStyle||focusStyle.focusBorderStyle||'#999999'):(item.blurBorderStyle||focusStyle.blurBorderStyle||'#f8f8f8'))}"
						 class="width100 border_radius_4px transition_point6s" :disabled="item.disabled" :maxlength="item.maxlength||140"
					...
					<view :class="[classObj.contentLayout]" :style="classObj.padding0_3 + classObj.contentWidth" v-else-if="item.type&&item.type=='picker-date'">
						<block v-if="pickerMode === 'arrowhead'">
							...
							<!-- #endif -->
								<view 
								class="flex_row_s_c width70 word_wrap" 
								...
								<view class="flex_row_s_c width30">
						...
						<block v-else>
							<view class="flex_row_none_c width100" v-if="inputsObj[item.variableName||(onLoadData+index)]">
								<view class="flex_row_s_c width70 fontColor666666 word_wrap" :style="classObj.contentFontSize">
									{{inputsObj[item.variableName||(onLoadData+index)]}}
								</view>
								<view class="flex_row_s_c width30">
									<!-- #ifndef H5 -->
							...
							<view class="flex_row_s_c" v-else>
								<!-- #ifndef H5 -->
								<button type="primary" @tap="showPicker"
								 :data-item="item"
								 :data-index="item.variableName||index"
						...
						<block v-if="pickerMode === 'arrowhead'">
							...
							<!-- #endif -->
								<block v-if="item.linkage">
									<view class="flex_row_s_c width70 word_wrap" 
					...
					<view :class="[classObj.contentLayout]" :style="classObj.padding0_3 + classObj.contentWidth" v-else-if="item.type&&item.type=='picker-custom2'">
						...
						<block v-else>
							<view class="flex_row_none_c width100" v-if="inputsObj[item.variableName||(onLoadData+index)]">
								<block v-if="item.linkage">
									<view class="flex_row_s_c width70 fontColor666666 word_wrap" :style="classObj.contentFontSize" v-if="item.linkageNum==2">
									...
									<view class="flex_row_s_c width70 fontColor666666 word_wrap" :style="classObj.contentFontSize" v-else-if="item.linkageNum==3">
					...
					<view :class="[classObj.contentLayout]" :style="classObj.padding0_3 + classObj.contentWidth" v-else-if="item.type&&item.type=='picker-provincialStreet'">
						...
						<block v-else>
							<view class="flex_row_none_c width100" v-if="inputsObj[item.variableName||(onLoadData+index)]">
								<view class="flex_row_s_c fontColor666666 width70 word_wrap" :style="classObj.contentFontSize">
									{{inputsObj[item.variableName||(onLoadData+index)].label}}
								</view>
								<view class="flex_row_s_c width30">
									<!-- #ifndef H5 -->
					...
					<view class="flex_row_none_c transition_point6s " :style="classObj.contentWidth + 'border-bottom: 1px solid ' + (focusStyleObj[(item.variableName||(onLoadData+index))]?(item.focusBorderStyle||focusStyle.focusBorderStyle||'#999999'):(item.blurBorderStyle||focusStyle.blurBorderStyle||'#f2f2f2')) + ';'" v-else>
						<view class="flex_row_none_c width100" :style="">
				...
				<view class="input_buttons" v-show="isConfigForm">
					<view class="flex solid-bottom justify-around">
						<view :key="index" class=" padding-sm margin-xs radius" v-for="(btnitem,index) in inputBut" @click="onInputBtn(item,btnitem)">
							<text class="lg text-blue" :class="btnitem.icon"></text>
							</view>
					</view>
				</view>
			</view>
		...
		<view class="flex_row width100"  :class="[animationType||'']" :style="classObj.padding2_5 + classObj.animationDuration1"
		 v-if="ifCode">
			 <view class="marginRight5 flex_row_s_c" :class="[titleFixedWidth?'width20':'Flex1_5', titleFixedWidth?'':'maxWidth40', classObj.titleLayout]"
			...
			<view class="flex_row" :class="[(titleFixedWidth?classObj.contentsWidth:'Flex4'), classObj.contentsLayout]">
				...
				<view class="flex_row_s_c Flex1" :style="classObj.padding0_3">
		...
		data() {
			...
			return {
				...
				classObj: { //拼接页面需用的样式
					...
					contentsLayout: this.titleHide?this.contentSet.layout=='left'?'flex_row_none_c':this.contentSet.layout=='center'?'flex_row_c_c':'flex_row_s_c':'flex_row_s_c',
					contentLayout: this.contentSet.layout=='left'?'flex_row_none_c':this.contentSet.layout=='center'?'flex_row_c_c':'flex_row_s_c',
					titleLayout: this.titleSet.layout=='left'?'flex_row_none_c':this.titleSet.layout=='center'?'flex_row_c_c':'flex_row_s_c',
